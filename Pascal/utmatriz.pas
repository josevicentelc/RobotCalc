...
begin
    result := true;
    if (rCount <> cCount) or (rCount = 0) then
       result := false
    else
...
begin
   ...
   begin
      ...
      begin
          ...
          N := result.getRow(j2);
          if ((N1.getValue(0, j) > 0) and (result.getValue(j2, j) > 0))
             or
             ((N1.getValue(0, j) < 0) and (result.getValue(j2, j) < 0 )) then
          begin
